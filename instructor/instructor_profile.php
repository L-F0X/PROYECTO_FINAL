...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !verify_csrf_token($_POST['csrf_token'] ?? '')) {
    ...
    if ($action === 'update_profile') {
        ...
        if ($nombre !== '' && $apellido !== '' && !preg_match('/^[a-zA-ZáéíóúÁÉÍÓÚüÜñÑ]+( [a-zA-ZáéíóúÁÉÍÓÚüÜñÑ]+)?$/u', $nombre)) {
            $message = '✗ El nombre solo debe contener letras y espacios.';
            $messageType = 'error';
        } elseif ($nombre !== '' && $apellido !== '' && !preg_match('/^[a-zA-ZáéíóúÁÉÍÓÚüÜñÑ]+( [a-zA-ZáéíóúÁÉÍÓÚüÜñÑ]+)?$/u', $apellido)) {
